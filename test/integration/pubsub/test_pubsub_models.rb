...
class TestPubsubModels < PubSubShared
  ...
  def test_topics_all
    ...
    retry_times(3) do
      ...
      if result.nil?
        raise StandardError.new("no topics found")
      end

      unless result.any? { |topic| topic.name == name }
        raise StandardError.new("failed to find expected topic")
      end
    end
  end
  ...
  def test_subscriptions_list
    ...
    retry_times(3) do
      ...
      if result.nil?
        raise StandardError.new("no subscriptions found")
      end

      unless result.any? { |subscription| subscription.name == subscription_name }
        raise StandardError.new("failed to find expected subscription")
      end
    end
  end
